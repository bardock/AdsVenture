...
namespace AdsVenture.Commons.Caching
{
    ...
    public class DeferredCacheProxyCollection<T>
    {
        ...
        public DeferredCacheProxyCollection(ICache cache, string keyPrefix, TimeSpan expiration = default(TimeSpan))
            : this(cache, keyPrefix, x => expiration) { }

        public DeferredCacheProxyCollection(ICache cache, string keyPrefix, Func<T, TimeSpan> expiration)
        {
            _cache = cache;
            if (keyPrefix == null)
        ...
        /// </remarks>
        public T GetData(Func<T> dataLoadFunc, params object[] @params)
        {
            string key = BuildKey(@params);
            return GetProxy(key).GetData(dataLoadFunc);
        }
